...
def convert_data(result: dict) -> ExtractedResult:
  ...
  for layout in result["layouts"]:
    ...
    if cls == LayoutClass.ISOLATE_FORMULA:
      layout = cast(Any, ExtendedFormulaLayout).from_dict(layout)
    elif cls == LayoutClass.TABLE:
      layout = cast(Any, ExtendedTableLayout).from_dict(layout)
    else:
      layout = cast(Any, ExtendedPlainLayout).from_dict(layout)
